#https://github.com/manitadayon/Auto_HMM/blob/main/Hidden_Markov_Model/DHMM.py
...
class DHMM_Model:
    def __init__(self,Train_ratio,Max_state,Iter,Feat,N,T,N_symb):
    ...
    def Best_States(self, Data_train, Test_data):
        self.Test_data = Test_data
        self.Train_Data = np.array(Data_train).reshape((-1,1)) # Convert to numpy array with one column

        self.Len=[self.T for ii in range(0,Data_train.shape[0])]   # Lengths must be list
...
class Supervised_DHMM(DHMM_Model):
    def AIC_BIC(self):
        ...
        Model = MultinomialHMM(n_components=self.Max_state, tol=pow(10,-5))

        # for xs in self.Train_Data:
        #     if(xs[0] < 0):
        #         xs[0] = 0
        # print( min(self.Train_Data) )

        Model = Model.fit(self.Train_Data,self.Len)
        AIC.append(-2 * Model.score(self.Train_Data) + 2 * self.num_params)
        BIC.append(-2 * Model.score(self.Train_Data) +  self.num_params * np.log(self.Train_Data.shape[0]))
